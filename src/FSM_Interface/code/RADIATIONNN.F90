...
real :: &
  ...
  afs                 ! Maximum albedo for fresh snow - local variable, constant or tuned depending on TNELEV
...
do i = 1, Nx
  ...
  if (OSHDTN == 0) then 
    ...
    if (dem(i,j) >= 2200) then
      afs  = 0.92
    else if (dem(i,j) <= 1400) then
      afs = 0.80
    else
  ...
  ! Prognostic
   ...
   ! end if 
    ...
    alim = (asmn/tau + Sf(i,j)*afs/Salb)/rt
