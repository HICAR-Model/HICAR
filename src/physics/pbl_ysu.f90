...
!
   ...
   do j = jts,jte
      ...
      do k = kts,kte
        do i = its,ite
          qv2d(i,k) = qv3d(i,k,j)
          if(ndiff.ge.2) qv2d(i,k+kte) = qc3d(i,k,j)
          if(ndiff.ge.3.and.flag_qi) qv2d(i,k+kte+kte) = qi3d(i,k,j)
        enddo
      enddo
...
!
     do k = kts,kte
       do i = its,ite
         ...
         if(ndiff.ge.2) rqcblten(i,k,j) = rqvbl2dt(i,k+kte)
         if(ndiff.ge.3.and.flag_qi) rqiblten(i,k,j) = rqvbl2dt(i,k+kte+kte)
       enddo
     enddo
!
   enddo
